...
namespace TS3AudioBot.Web
{
	...
	public sealed class WebAccountService
	{
		...
		public bool IsInitialized => accounts.Count() > 0;
		...
		{
			if (IsInitialized) { error = "系统已完成初始化。"; return false; }
		...
		public string? Login(string username, string password)
		{
			...
			if (account == null || !account.Enabled || !Verify(password, account)) return null;
			var id = Convert.ToBase64String(RandomNumberGenerator.GetBytes(32));
			sessions.Insert(new WebSession { Id = id, Username = account.Username, ExpiresAt = DateTime.UtcNow.AddDays(14) });
			return id;
		}
		...
		public WebAccount? GetAccount(string? sessionId)
		{
			...
			return account?.Enabled == true ? account : null;
		}

		public void Logout(string? sessionId) { if (!string.IsNullOrEmpty(sessionId)) sessions.Delete(sessionId); }
